...
export const random = (req, res) => {
  const maxLength =
    Math.sign(req.query.max_length) === 1 ? req.query.max_length : null;
...
export const list = (req, res) => {
  ...
  const data = db
    .prepare(
      `
        SELECT id, fact, length(fact) length from facts
        ${maxLength ? withMaxLength : ''}
        LIMIT ${limit}
